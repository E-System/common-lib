...
class HashSpec extends Specification {
    ...
    def "BCrypt"() {
        expect:
        Hash.bcrypt().of("Hello").startsWith('$2a$12$')
    }

    def "CRC32"() {
        expect:
        Hash.crc32().of("Hello") == 4157704578
    }

    def "CRC16"() {
        expect:
        Hash.crc16().of("Hello") == 56026
    }

    def "SHA-1"() {
        expect:
        Hash.sha1().of("Hello") == 'f7ff9e8b7bb2e09b70935a5d785e0cc5d9d0abf0'
    }

    def "hmacSha256"() {
        expect:
        Hash.hmacSha256("secret_key").of("Test content") == "oKL8Wjg+l73byfPL1FIYcV2T1IcpAHhopwScTcOoSIU="
    }

    def "crc16ccitt required not null value"() {
        when:
        Hash.crc16ccitt().of(null as byte[])
        Hash.crc16ccitt().of(null as String)
        then:
    ...
    def "crc16ccitt with skip must be correct without IndexOutException "() {
        expect:
        Hash.crc16ccitt(skipIdx, skipLen).of(src) == Hash.crc16ccitt().of(target)
        where:
